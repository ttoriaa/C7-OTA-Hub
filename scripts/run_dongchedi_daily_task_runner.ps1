...
$logPath = Join-Path $logDir ("charging_task_{0}.log" -f (Get-Date -Format "yyyy-MM-dd_HHmmss"))
$lockPath = Join-Path $logDir "charging_task.lock"
$outTmp = Join-Path $logDir ("charging_task_stdout_{0}.tmp" -f ([guid]::NewGuid().ToString("N")))
$errTmp = Join-Path $logDir ("charging_task_stderr_{0}.tmp" -f ([guid]::NewGuid().ToString("N")))
...
try {
    ...
    "[$(Get-Date -Format s)] SKIP charging task because another instance is running" | Out-File -FilePath $logPath -Encoding UTF8 -Append
    exit 0
}

"[$(Get-Date -Format s)] START charging task" | Out-File -FilePath $logPath -Encoding UTF8 -Append

try {
    for ($generateAttempt = 1; $generateAttempt -le 2; $generateAttempt++) {
        "[$(Get-Date -Format s)] generate attempt=$generateAttempt" | Out-File -FilePath $logPath -Encoding UTF8 -Append
    ...
    if ($publishExitCode -ne 0) {
        ...
        "[$(Get-Date -Format s)] DONE charging task with publish warning" | Out-File -FilePath $logPath -Encoding UTF8 -Append
        exit 0
    }

    "[$(Get-Date -Format s)] DONE charging task" | Out-File -FilePath $logPath -Encoding UTF8 -Append
    exit 0
}
finally {
    if (Test-Path $outTmp) {
